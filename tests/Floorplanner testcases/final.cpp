...
void parseTechLef(string filename, unordered_map<int, float> &layerPitch, float &minPitch, string &unit) {
	ifstream inFile;
	...
	if (!inFile) {
		...
		minPitch = INT_MAX;
		for (int i = 0; i<layerPitch.size(); i++) {
			if (layerPitch[i] < minPitch)
				minPitch = layerPitch[i];
		}
	}
}
...
int main()
{
	...
	int init_W;
	bool io_del = 0;
	string file_name = "test11.txt";
	unordered_map < int, HardR> HM;
	unordered_map < int, SoftR> SM;
	unordered_map < int, HardR> HMdum;
	unordered_map < int, SoftR> SMdum;
	unordered_map < int, io> IOM;
	unordered_map < int, io> IOMtemp;
	int dum = 0;
	double area = 0;
	float totalw = 0;
	float totalh = 0;
	...
	//parser(file_name, HMdum, SMdum, pin_count, pin_width, pin_height, dum, dum);
	...
	pins_min_width = pins_side * (pin_max + iomar);
	...
	for (int i = 1; i <= pins_side; i++)
	{
		io temp;
		temp.x = 0;
		temp.y = i * (pin_min + iomar);
		IOMtemp[iocountt] = temp;
		iocountt++;
	}
	...
	try {
		...
		{
			/*for (auto it = IOMtemp.begin(); it != IOMtemp.end(); ++it)
			{
				if (rand()%2)
				{
					c.add(IloAbs(vars[i] - it->second.x) - IloAbs(vars[i + 1] - it->second.y) - maxY <= 0);
				}
			}*/
			int rr = rand() % 4;
			if (rr == 0)
				c.add(IloAbs(vars[i] - 0 ) + IloAbs(vars[i + 1] - 0) - maxY <= 0);
			else if (rr == 1)
				c.add(IloAbs(vars[i] - 0) + IloAbs(vars[i + 1] - W) - maxY <= 0);
			else if (rr == 2)
				c.add(IloAbs(vars[i] - W) + IloAbs(vars[i + 1] - 0) - maxY <= 0);
			else if (rr  == 3)
				c.add(IloAbs(vars[i] - W) + IloAbs(vars[i + 1] - W) - maxY <= 0);
		}
	/*	for (int c = hard_count * 3 + 1; c < ct; c += 4)
		...
		model.add(c);
		model.add(IloMinimize(env, vars[0] + maxX + 10000*maxY));
		...
		if (true)
		{
			...
			for (int i = 1; i <= pins_side; i++)
			{
				Results << "0 , " << i * (pin_min+iomar) <<endl;
				io temp;
				temp.x = 0;
				temp.y = i * (pin_min + iomar);
				IOM[iocount] = temp;
			...
			for (int i = 1; i <= pins_side; i++)
			{
				Results << i * (pin_min + iomar) << " , 0" << endl;
				io temp;
				temp.x = i * (pin_min + iomar);
				temp.y = 0;
				IOM[iocount] = temp;
			...
			for (int i = 1; i <= pins_side; i++)
			{
				Results << W << " , " << i * (pin_min + iomar) << endl;
				io temp;
				temp.x = W + 2*iomar + 2*pinmar;
				temp.y = i * (pin_min + iomar);
				IOM[iocount] = temp;
			...
			for (int i = 1; i <= pins_side; i++)
			{
				Results << i * (pin_min + iomar) << " , "<< realval[0] << endl;
				io temp;
				temp.x = i * (pin_min + iomar);
				temp.y = hfinal + 2 * pinmar+ iomar ;
				// + 2*iomar + 2*pinmar
				IOM[iocount] = temp;
				iocount++;
			}
			...
			def << "DIEAREA(0 0) ( " << W + 2 * pin_max << " " << hfinal << " ) ;" << endl;
			...
			def << "PINS  " << pin_count << " ;" << endl;
			int coun = 0;
			for (auto it = IOM.begin(); it != IOM.end(); ++it)
			{
				def << "- pin" <<coun<< " + NET "<< "- pin" << coun<< endl;
				def << " + LAYER metal1 ( 0 0 ) ( 0 0 )" << endl;
				def << "  + PLACED ( " << it->second.x << " " << it->second.y << " ) N ;" << endl;
				coun++;
			...
			for (int i = ct; i < (ct + soft_count * 4); i += 4)
			{
				...
				SM[i / 4 - hard_count -1  ].xcor = legalize(realval[i] + pin_max, gmin);
				SM[i / 4 - hard_count  -1].ycor = legalize(realval[i+1] + pin_max,gmin);
				SM[i / 4 - hard_count -1 ].width = legalize(realval[i+2] , gmin);
				SM[i / 4 - hard_count -1 ].height = legalize( realval[i+3] , gmin);

			}
			cout << "Values = " << vals << endl;

			
			ofstream POout("PinAssignmentOut.txt");
			//vector<Pin> vec;
			for (auto it = HM.begin(); it != HM.end(); ++it)
			{
				POout << it->second.name << "		" << it->second.width << "		" << it->second.height << "		" << it->second.xcor << "		" << it->second.ycor << endl;
				POout << "pins:" << endl;

				for (int i = 0; i < it->second.pins.size(); i++)
				{
					POout << "		" << it->second.pins[i].name << "		" << it->second.pins[i].metal << "		" << it->second.pins[i].xcor << "		" << it->second.pins[i].ycor << endl;
				}
	...
	int N = (W + 2 * pin_max + 2*iomar)/gmin;
	int M = (hfinal + 2*iomar) /gmin;
	...
	for (auto it = IOM.begin(); it != IOM.end(); ++it)
	{
		int x = it->second.x / gmin;
		int y = it->second.y / gmin;
	
		for (int i = x; i < x + pin_width; i++)
			for (int j = y; j < y + pin_height; j++)
			{
				ary[i][j] = 1;
			}
	}
